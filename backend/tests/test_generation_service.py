...
class SuccessfulProvider:
    ...
    def generate(self, request):
        ...
        text = (
            ...
            "Вам будет проще продвинуться вперед, если выбрать одно главное дело. "
            "Ваше внимание к деталям поможет избежать лишней суеты. "
            f"Внутренний тестовый вариант {self.calls} сохранен."
        )
...
class ForbiddenSignProvider:
    name = "test-forbidden-sign-provider"
    model_name = "test-forbidden-sign-model"
    ...
    def generate(self, request):
        self.calls += 1
        text = "Овнов сегодня ждет удача, но им стоит избегать спешки."
...
def _messages_content(request_payload):
    return "\n".join(
        message["content"] for message in request_payload.get("messages", [])
    )
...
def test_run_daily_generation_creates_run_items_attempts_and_forecasts(app, monkeypatch):
    ...
    with app.app_context():
        ...
        for attempt in attempts:
            ...
            content = _messages_content(request_payload).lower()

            assert request_payload["prompt_version"] == "daily-ru-v1"
            assert request_payload["messages"]
            assert request_payload["metadata"]["sign_key"] in {"aries", "taurus"}
            assert request_payload["metadata"]["target_date"] == target_day.isoformat()
...
def test_run_daily_generation_records_retryable_provider_failures(app, monkeypatch):
    ...
    with app.app_context():
        ...
        assert all(attempt.request_payload["messages"] for attempt in attempts)

        assert Forecast.query.count() == 0
        assert provider.calls == 2


def test_run_daily_generation_retries_and_rejects_forbidden_zodiac_text(app, monkeypatch):
    provider = ForbiddenSignProvider()
    _install_provider(monkeypatch, provider)
    target_day = date(2026, 5, 13)
    ...
    with app.app_context():
        ...
        assert item.status == "failed"
        assert item.forecast_id is None
        assert "forbidden zodiac terms" in item.error_message
        ...
        assert [attempt.status for attempt in attempts] == ["failed_retryable", "failed"]
        assert all(attempt.raw_response for attempt in attempts)
        assert all("Овнов" in attempt.raw_response for attempt in attempts)
        assert all("forbidden zodiac terms" in attempt.error_message for attempt in attempts)
